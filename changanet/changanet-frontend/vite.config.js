...
export default defineConfig({
  ...
  server: {
    ...
    headers: {
      ...
      'Content-Security-Policy': "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.gstatic.com https://maps.googleapis.com https://maps.gstatic.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; connect-src 'self' http://localhost:3002 http://localhost:3003 ws://localhost:3002 ws://localhost:3003 wss://localhost:3002 wss://localhost:3003 https://maps.googleapis.com https://places.googleapis.com https://securetoken.googleapis.com https://www.googleapis.com https://identitytoolkit.googleapis.com https://o4510260990574592.ingest.us.sentry.io;",
    ...
    proxy: {
      '/api': {
        target: process.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:3002',
        changeOrigin: true,
        secure: false,
      },
      '/health': {
